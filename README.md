...
Фреймворк позволяет инвесторам оптимально распределять капитал среди российских акций с учетом стохастической динамики рынка, проводить анализ чувствительности к различным шокам и количественно оценивать хвостовые риски (VaR, CVaR).
...
|---------|---------|--------|
| **Data Loading** | Загрузка исторических данных с MOEX ISS API | ✅ Готово |
| **GARCH Modeling** | Калибровка параметров волатильности методом MLE | ✅ Готово |
| **Asset Filtration** | Фильтрация активов по критериям стационарности | ✅ Готово |
| **HJB Optimization** | Аналитическое решение задачи Мертона | ✅ Готово |
| **Dividend Yield** | Расчет полной доходности (капитал + дивиденды) | ✅ Готово |
| **Portfolio Constraints** | No short sales + Full investment constraints | ✅ Готово |
| **Monte Carlo Sim** | Векторизированная симуляция 5000+ траекторий | ✅ Готово |
| **Risk Metrics** | VaR, CVaR, Sharpe Ratio, Max Drawdown | ✅ Готово |
| **Stress-Testing** | 8 сценариев (базовый, кризис, черный лебедь и т.д.) | ✅ Готово |
| **Sensitivity Analysis** | Анализ чувствительности к γ и временным горизонтам | ✅ Готово |
| **Streamlit Dashboard** | Интерактивный веб-интерфейс | 🚧 В разработке |
| **ML Forecasting** | LSTM/Transformer прогноз доходностей | 🚧 Запланировано |
...
На основе данных MOEX за период **01.11.2024 – 01.11.2025** (10 акций: SBER, GAZP, YDEX, TATN, MOEX, PHOR, NVTK, LKOH, PLZL, VTBR):
...
Портфель демонстрирует устойчивость в умеренных кризисных сценариях, но сильно деградирует при экстремальных событиях, где Sharpe коллапсирует до 0.0584. Это указывает на наличие левого хвоста в распределении доходности.
...
✅ **DO:**
- Использовать минимум 2-3 года исторических данных (500+ торговых дней)
- Проверять наличие пропусков и синхронизировать даты
- Исключать дни с аномальными объемами торговли

❌ **DON'T:**
- Не использовать данные менее чем за 6 месяцев
- Не смешивать разные временные интервалы (daily + weekly)
- Не игнорировать корпоративные действия (сплиты, дивиденды)

### 2. GARCH-моделирование

✅ **DO:**
- Проверять условие стационарности: α + β < 0.995
- Визуализировать условную волатильность
- Сравнивать модели GARCH(1,1) и GARCH(2,1)

❌ **DON'T:**
- Не использовать активы с α + β ≥ 0.99 (интегрированная GARCH)
- Не пропускать диагностику остатков
- Не переоценивать стабильность параметров GARCH

### 3. Портфельная оптимизация

✅ **DO:**
- Применять регуляризацию (Tikhonov, shrinkage)
- Проверять условие число обусловленности матрицы Σ
- Пересчитывать веса ежемесячно

❌ **DON'T:**
- Не использовать истинное решение Мертона без ограничений (риск концентрации)
- Не игнорировать эффект ошибок в оценке µ
- Не доверять весам с < 1% в портфеле

### 4. Монте-Карло симуляция

✅ **DO:**
- Использовать M ≥ 5000 траекторий для стабильности
- Проверять сходимость на подвыборках (M=1000, 5000, 10000)
- Сохранять seed для воспроизводимости

❌ **DON'T:**
- Не использовать M < 1000 (плохая сходимость квантилей)
- Не игнорировать начальное условие симуляции
- Не забывать про ежегодную переиндексацию коэффициентов

### 5. Стресс-тестирование

✅ **DO:**
- Проверять исторические шоки (2008, 2020, 2022)
- Анализировать несколько временных горизонтов
- Комбинировать базовые и экстремальные сценарии

❌ **DON'T:**
- Не полагаться на один сценарий для оценки риска
- Не игнорировать tail dependence между активами
- Не забывать про черные лебеди (вероятность ~1% в год)
...
- [x] GARCH(1,1) моделирование
- [x] HJB решение задачи Мертона
- [x] Монте-Карло симуляция
- [x] Стресс-тестирование (8 сценариев)
- [x] Анализ чувствительности

### Phase 2: Advanced Models
- [ ] Merton Jump-Diffusion (MJD) процессы
- [ ] Variance Gamma и CGMY модели
- [ ] Hidden Markov Models (HMM) для режимов
- [ ] Lévy процессы с calibration
- [ ] Multi-factor model (Fama-French 5-factor)

### Phase 3: Portfolio Enhancements
- [ ] Ограничения на концентрацию ($w_i \leq w_{\max}$)
- [ ] Секторное распределение капитала
- [ ] Транзакционные издержки и спред
- [ ] Динамическое переразбалансирование
- [ ] Robustness optimization (DRO)

### Phase 4: Backtesting & Analytics
- [ ] Historical backtesting framework
- [ ] Walk-forward optimization
- [ ] Sharpe ratio из бэктеста с реальными торговлями
- [ ] Slippage и commission моделирование
...
- [ ] Telegram alerts для шоков
- [ ] Excel add-in для интеграции

### Phase 6: ML & AI
- [ ] LSTM/Transformer прогноз доходностей
- [ ] Anomaly detection (Isolation Forest, LOF)
- [ ] Reinforcement Learning для динамического управления
- [ ] Deep learning для калибровки волатильности
...
- **Issues:** Используйте GitHub Issues для отчетов об ошибках [Issue](https://github.com/russiankendricklamar/quantitative-risk-engine/issues)
- **Discussions:** Используйте GitHub Discussions для вопросов [Discussions](https://github.com/russiankendricklamar/quantitative-risk-engine/discussions)
